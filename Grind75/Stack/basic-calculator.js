...
const calc20250822 = (s) => {
    ...
    let unary = true    // unary is only checked at start and any index right after open bracket.
    while (i < n) {
        if (s[i] === ' ') {
            i += 1
            continue
        ...
        if (unary === true) {   // checked right away on the current index
            unary = false
            if (s[i] === '+' || s[i] === '-') {
                // -5 becomes 0-5
                operandSk.push(0)
        ...
        if (isNaN(s[i]) === false) {
            const res = getNumber(i, s)
            i = res[0]
            operandSk.push(res[1])
        } else if (s[i] === '(') {
            operatorSk.push(s[i])
            unary = true    // possible unary after open bracket, need to check on next index
            i += 1
        } else if (s[i] === ')') {
